...
void CVehicleMovementArcadeWheeled::ProcessMovement(const float deltaTime)
{
	...
	if (isAIDisabled || isDriverHidden || !((m_actorId || m_remotePilot) && m_isEnginePowered) || isDestroyed || IsMovementProcessingEnabled()==false)
	{
...
void CVehicleMovementArcadeWheeled::InternalPhysicsTick(float dt)
{
	...
	}
	{
		...
		for (int i=0; i<numWheels; i++)
		{
			...
			maxLateralImpulse[i].min *= m_handling.compressionScale * contact * gravityScale;
			maxLateralImpulse[i].max *= m_handling.compressionScale * contact * gravityScale;
		}

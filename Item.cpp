		...
		if (ShouldDoPostSerializeReset())
		{
			if (IInventory* pInventory = pOwner->GetInventory())
			{
...

				{
					...
					//2) No current item yet (we always need one, and the correct 'selected' one will be serialized after this one
					//3) The current item will never be auto selected on pick up, like binoculars, pick&throw or noweapon (this code path is not executed for it)
